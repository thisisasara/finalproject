...
function mousePressed() {
 for(var i=0;i<hBlocks.length;i++){
    ...
    if(hBlocks[i].classList.contains("ball")){
      blocks.push(new Ball(mouseX, mouseY, hBlocks[i].offsetWidth, hBlocks[i].offsetHeight));
    }else 
    if(hBlocks[i].classList.contains("block")){
      blocks.push(new Box(mouseX, mouseY, hBlocks[i].offsetWidth, hBlocks[i].offsetHeight));
    }
  }
}
